...
def test_exchange_data():
    moex.make_request = Mock()
    moex.make_request.return_value = json.loads(open(os.path.dirname(os.path.realpath(__file__))+"/moex_ex_data_in.json", 'r').read())
    assert moex.exchange_data() == json.loads(open(os.path.dirname(os.path.realpath(__file__))+"/moex_ex_data_out.json", 'r').read())
