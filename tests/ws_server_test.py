...
def test_make_response_func_successful():
    ...
    s.make_response(websocket=websocket, func=f, thread_id="0x01")

    websocket.send.assert_called_with(json.dumps({"data": "some data..."}))
    assert "0x01" in s.sockets[websocket.id]


def test_make_response_err_msg():
    websocket = SocketConnection()
    s.sockets[websocket.id] = set()
    s.sockets[websocket.id].add("0x01")
    websocket.send = Mock()
    s.make_response(websocket, thread_id="0x01", err_msg='some err...')

    websocket.send.assert_called_with('some err...')
    assert ("0x01" not in s.sockets[websocket.id]) is True


def test_make_response_func_all_exc():
    websocket = SocketConnection()
    s.sockets[websocket.id] = set()
    s.sockets[websocket.id].add("0x01")
    ...
    for i in range(len(exc_.exceptions)):
        ...
        s.make_response(websocket=websocket, func=f, thread_id="0x01", err_msg="", exc=e)
        s.sockets[websocket.id].add("0x01")
...
def test_manage_required_parameter_type_missed():
    ...
    t = test_request.copy()
    t.pop("type")
    s.make_response = Mock()
    s.manage(t, websocket)
    s.make_response.assert_called_with(websocket, err_msg=s.make_error_msg("RPM 0x01"))
...
def test_manage_required_parameter_data_missed():
    ...
    s.make_response = Mock()
    s.manage(t, websocket)
    s.make_response.assert_called_with(websocket, err_msg=s.make_error_msg("RPM 0x02"))
...
def test_manage_wpv0x04():
    ...
    s.make_response = Mock()
    s.manage(t, websocket)
    s.make_response.assert_called_with(websocket,  err_msg=s.make_error_msg("WPV 0x04"))
...
def test_manage_wpv0x05_bad_count_or_timeout_value_type():
    ...
    s.make_response = Mock()
    s.manage(t, websocket)
    s.make_response.assert_called_with(websocket, err_msg=s.make_error_msg("WPV 0x05"))
...
def test_send_ticker_data_successful():
    ...
    threading.Thread.assert_called_with(target=s.make_response, args=_args, kwargs=_kwargs)
...
def test_handle_wrf0x01_type_error():
    ...
    s.manage, s.make_response = Mock(), Mock()
    s.handle(websocket)
    s.make_response.assert_called_with(websocket, err_msg=s.make_error_msg("WRF 0x01"))
...
def test_handle_wrf0x02_request_is_not_dict():
    ...
    s.manage, s.make_response = Mock(), Mock()
    s.handle(websocket)
    s.make_response.assert_called_with(websocket, err_msg=s.make_error_msg("WRF 0x02"))
